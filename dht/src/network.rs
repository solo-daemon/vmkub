...
use crate::routing::FindValueResultAttribute;

use super::attributes::DHTValueStruct;
use super::key::Key;
use super::node::*;
...
pub enum Request {
    Ping,
    Store_Tuple(Key, NodeInfo),
    Store_Attribute(Key, DHTValueStruct),
    FindNode(Key),
    FindValue(Key),
    FindValueAttr(Key),
...
pub enum Response {
    ...
    FindValueAttr(FindValueResultAttribute),
}
